...
- 隠しファイル・隠しフォルダーを一覧に表示するようにしました。表示(&V)メニューの
  「隠しファイルを表示」（Ctrl+H）で切り替えられ、設定に保存されます。タブ表示と
  カラム表示の両方に効きます。
- 表示・非表示の判定を Windows Explorer に合わせました。`Thumbs.db` のような
  「保護されたオペレーティングシステムファイル」（隠し属性とシステム属性の両方が
  立っている項目）は、誤操作を防ぐため上記の切り替えに関わらず表示しません。
- スクロールバーの操作を Windows Explorer に合わせました。タブ表示のファイル一覧の
  縦スクロールバーをウィンドウ右端に密着させたため、最大化時は画面の一番右でも
  掴めます。また、スクロールバーのトラックを Shift+クリックすると、その位置へ
  ジャンプします。

### Fixed

- フォルダをダブルクリックしたときに、インストール先フォルダがエクスプローラーで開いてしまう問題を修正しました。
- 外部アプリでファイルが移動・削除されたときに、タイル表示のスクロール位置が上へ飛ぶ問題を修正しました。
- サムネイルに使えるアイテムが無いフォルダのアイコンが、サムネイルより小さく表示される問題を修正しました。

...
def test_main_window_tab_current_path_change_clears_summary(
    monkeypatch,
    qtbot,
    tmp_path: Path,
) -> None:
    ...
    window._tab_container.currentPathChanged.emit(tmp_path)

    assert window._status_summary == BrowserStatus()
    assert window._status_detail_label.text() == "0個の項目（フォルダ0個/ファイル0個）"
